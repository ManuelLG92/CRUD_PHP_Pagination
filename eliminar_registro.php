...
<body>
    ...
    <?php 
    //TO-DO: quede controlando el id pasado por la url que se obtiene con GET
    ...
    if(isset($_POST['enviar_x'])){
        $idToDelete = $_POST['idQuery'];
        $sentencia = "DELETE FROM contactes WHERE id='". $idToDelete . "'";
        $ancestorPageInput = $_POST['ancestorPage'];
        $numeroPaginaAncestorInput = $_POST['ancestorPageNumber'];
        $pageAndNumber = '?pagina='.trim($numeroPaginaAncestorInput);
        $ancestorPageWithNumer = $ancestorPageInput .$pageAndNumber;
    ...
    <?php if($successDelete):?>
        <h1>Registro eliminado satisfactoriamente.</h1>
        <div>
    ...
    <?php if($deleteError):?>
        <h1>Ha habido un error eliminando el registro, intentalo de nuevo mas tarde.</h1>
        <a href="<?php echo $ancestorPageWithNumer ?>"></a>
        <?php echo exit; ?>
    <?php endif?>
        <?php if($_GET): ?>
            <?php if($idOnRequest && !$idRegistroInvalido): ?>
                <span class="tituloCrud">ELIMINAR REGISTRO Nº: <?php echo $idQuery ?> </span>
                <hr>
                <div class="centrar">
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']) ?>" method="post">
                        ...
                        <div>
                            ...
                            <input type="image" src="./media/apply.png" class="imagenUltimaFila" alt="Eliminar" title="Eliminar registro" name="enviar">
                            <input type="image" class="imagenUltimaFila" src="./media/cancel.png" alt="Cancelar" title="Cancelar, vuelve a la pagina de origen." name="cancel">
